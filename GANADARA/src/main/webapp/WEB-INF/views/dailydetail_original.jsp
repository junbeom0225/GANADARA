...
<style type="text/css">
#main{position:relative; height:60%;}
</style>
</head>
...
<div id="main">
	...
	<table border="1">
		<tr>
			<th>writer ${login.member_role }</th>
...
<!-- 	 	<tr>
			<td colspan="2" align="right">
				<input type="button" value="modify" onclick="location.href='./updateform.do?db_no=${dbdto.db_no}&member_no=${dbdto.member_no }'"/>
	...
	<tr>
			<td colspan="2" align="right">
				<c:if test="${login.member_role eq 'USER' }">
					<input type="button" value="modify" onclick="location.href='./updateform.do?db_no=${dbdto.db_no}&member_no=${dbdto.member_no }'"/>
					<input type="button" value="delete" onclick="location.href='./delete.do?db_no=${dbdto.db_no}&member_no=${dbdto.member_no }'"/>
					<input type="button" value="list" onclick="location.href='./list.do'"/>
				</c:if>
				<c:if test="${login.member_role eq 'MENTO' } || ${login.member_role eq 'ADMIN' } ">
					<input type="button" value="list" onclick="location.href='./studentList.do?member_no=${login.member_no }'"/>
				</c:if>
			</td>
		</tr> 	
	</table>
		...
		$.ajax({
			...
			success : function(data){
				...
				$.each(data, function(key, value){
	                ...
	                a += '<div class="commentContent'+value.ma_no+'"> <textarea rows="10" cols="50"> 내용 : '+value.ma_content +'</textarea>';
	                a += '</div></div>';
				});
				$(".commentList").html(a);
			}
